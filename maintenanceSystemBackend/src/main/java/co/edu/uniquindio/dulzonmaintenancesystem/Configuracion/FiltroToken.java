    ...
    protected void doFilterInternal(
        ...
        if (request.getMethod().equals("OPTIONS")) {
            ...
            try {
                ...
                //Si la petición es para la ruta /api/operador se verifica que el token exista y que el rol sea OPERADOR
                if (requestURI.startsWith("/api/operador")) {
                    error = validarToken(token, Rol.OPERADOR);
                } else {
                    error = false;
                }
